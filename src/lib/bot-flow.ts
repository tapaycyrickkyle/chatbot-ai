...
export type BotFlowNodeConfig = {
  ...
  captureNextReply: boolean;
  replyTargetNodeId: string;
  position: {
    x: number;
    y: number;
  };
...
export function createDefaultBotFlowNodeConfig(seed?: Partial<BotFlowNodeConfig>): BotFlowNodeConfig {
  return {
    ...
    captureNextReply: Boolean(seed?.captureNextReply),
    replyTargetNodeId: seed?.replyTargetNodeId?.trim() || "",
    position: {
...
export function parseBotFlowNodeConfig(value: string, fallbackTitle?: string): BotFlowNodeConfig {
  ...
  try {
    ...
    return createDefaultBotFlowNodeConfig({
      ...
      captureNextReply: Boolean(parsed?.captureNextReply),
      replyTargetNodeId:
        typeof parsed?.replyTargetNodeId === "string" ? parsed.replyTargetNodeId : "",
      position:
